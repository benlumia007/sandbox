...
def virtualbox_path()
  ...
  if Vagrant::Util::Platform.windows? || Vagrant::Util::Platform.cygwin?
      ...
      if !@vboxmanage_path && ( ENV.key?( "VBOX_INSTALL_PATH" ) || ENV.key?("VBOX_MSI_INSTALL_PATH" ) )
...
Vagrant.configure( "2" ) do | config |
  ...
  config.vm.synced_folder "sites", "/srv/www", :owner => "www-data", :mount_options => [ "dmode=0775", "fmode=0774" ]
  ...
  sandbox_config['sites'].each do | site, args |
    if args['local_dir'] != File.join( vagrant_dir, 'sites', site ) then
      config.vm.synced_folder args['local_dir'], args['vm_dir'], :owner => "www-data", :mount_options => [ "dmode=0775", "fmode=0774" ]
    end
  end
  ...
  config.vm.provider :hyperv do | vm, override |
    ...
    override.vm.synced_folder "sites", "/srv/www", :owner => "www-data", :group => "vagrant", :mount_options => [ "dir_mode=0775", "file_mode=0774" ]
    override.vm.synced_folder "log/php", "/var/log/php", :owner => 'vagrant', :mount_options => [ "dir_mode=0777", "file_mode=0777" ]

    sandbox_config['sites'].each do | site, args |
      if args['local_dir'] != File.join( vagrant_dir, 'sites', site ) then
        override.vm.synced_folder args['local_dir'], args['vm_dir'], :owner => "www-data", :group => "vagrant", :mount_options => [ "dir_mode=0775", "file_mode=0774" ]
      end
    end
  end
