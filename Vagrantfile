...
Vagrant.configure( "2" ) do | config |
  ...
  config.vm.synced_folder "sites", "/srv/www", :owner => "vagrant", :mount_options => [ "dmode=775", "fmode=774" ]

  # This section when set, it will synced a folder that will use vagrant as default.
  sandbox_config['sites'].each do | site, args |
    if args['local_dir'] != File.join( vagrant_dir, 'sites', site ) then
      config.vm.synced_folder args['local_dir'], args['vm_dir'], :owner => "vagrant", :mount_options => [ "dmode=775", "fmode=774" ]
    end
  end
